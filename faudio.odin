...
when ODIN_OS == .Windows {
	foreign import lib "lib/windows/FAudio.lib"
} else {
	foreign import lib "system:FAudio"
}

FAudio :: distinct rawptr
FAudioVoice :: distinct rawptr
FAudioSourceVoice :: distinct FAudioVoice
FAudioSubmixVoice :: distinct FAudioVoice
FAudioMasteringVoice :: distinct FAudioVoice
FAudioEngineCallback :: distinct rawptr
FAudioVoiceCallback :: distinct rawptr

/* Enumerations */

FAudioDeviceRoles :: enum c.int {
	FAudioNotDefaultDevice            = 0,
	FAudioDefaultConsoleDevice        = 1,
	FAudioDefaultMultimediaDevice     = 2,
	FAudioDefaultCommunicationsDevice = 3,
	FAudioDefaultGameDevice           = 4,
	FAudioGlobalDefaultDevice         = 5,
	FAudioInvalidDeviceRole           = ~FAudioGlobalDefaultDevice,
}

FAudioDeviceRole :: bit_set[FAudioDeviceRoles;c.int]

FAudioFilterType :: enum {
	FAudioLowPassFilter,
...
FAudioProcessor :: distinct u32
FAUDIO_DEFAULT_PROCESSOR: FAudioProcessor : 0xFFFFFFFF
...
FAudioFilterParametersEXT :: struct #packed {
	Type:      FAudioFilterType,
	Frequency: f32, /* [0, FAUDIO_MAX_FILTER_FREQUENCY] */
	OneOverQ:  f32, /* [0, FAUDIO_MAX_FILTER_ONEOVERQ] */
	WetDryMix: f32, /* [0, 1] */
...
FAUDIO_MIN_FREQ_RATIO :: (1.0 / 1024.0)
...
FAUDIO_DEFAULT_FILTER_TYPE :: FAudioFilterType.FAudioLowPassFilter
...
FAUDIO_ABI_VERSION :: 0
FAUDIO_MAJOR_VERSION :: 24
FAUDIO_MINOR_VERSION :: 2
...
foreign lib {
  ...
  */
	...
	// FAudioCOMConstructEXT::proc(FAudio **ppFAudio, uint8_t version) -> u32 ---
  ...
  * Returns 0 on success.
  */
	//   FAudio_RegisterForCallbacks::proc(
	//    audio:FAudio,
	...
	//   * Returns 0 on success.
	//   */
	//   FAudio_UnregisterForCallbacks::proc(
	//    audio:FAudio,
	//    FAudioEngineCallback *pCallback
	//  ) ---
  ...
  */
	//   FAudio_CreateMasteringVoice8::proc(
	//    audio:FAudio,
  ...
  */
	...
	//  FAUDIODEPRECATED::proc("This function will break your program! Use FAudio_CommitOperationSet instead!")
	//   FAudio_CommitChanges::proc(audio:FAudio);
  ...
  *
  * EffectIndex:		The index of the effect ::proc(based on the chain order).
  ...
  *
  * EffectIndex:	The index of the effect ::proc(based on the chain order).
  * pEnabled:	Filled with either 1 ::proc(Enabled) or 0 ::proc(Disabled).
  *
  * Returns 0 on success.
  */
  ...
  * This is typically only used for panning or 3D sound ::proc(via F3DAudio).
  *
  * pDestinationVoice:	An output voice from the voice's send list.
  ...
  * Returns 0 on success.
  */
	//   FAudioVoice_SetOutputMatrix::proc(
	//    voice:FAudioVoice,
  ...
  * pLevelMatrix:	A float[SourceChannels * DestinationChannels].
  */
	//   FAudioVoice_GetOutputMatrix::proc(
	//    voice:FAudioVoice,
	//    FAudioVoice *pDestinationVoice,
	//    uint32_t SourceChannels,
	//    uint32_t DestinationChannels,
  ...
  * pBufferWMA:	See FAudioBufferWMA for details. ::proc(Also, don't use WMA.)
  *
  * Returns 0 on success.
  */
  ...
  */
	...
	/* Sets the frequency ratio ::proc(fancy phrase for pitch) of this source.
  *
  ...
  */
	...
	/* Requests the frequency ratio ::proc(fancy phrase for pitch) of this source.
  *
  ...
  * Returns 0 on success.
  */
	//   FAudioMasteringVoice_GetChannelMask::proc(
	//    FAudioMasteringVoice *voice,
	//    uint32_t *pChannelMask
	//  ) -> u32 ---


	//   FAudioCreateWithCustomAllocatorEXT::proc(
	//    FAudio **ppFAudio,
	//    uint32_t Flags,
	...
	//  ) -> u32 ---
	//   FAudioCOMConstructWithCustomAllocatorEXT::proc(
	//    FAudio **ppFAudio,
	//    uint8_t version,
	//    FAudioMallocFunc customMalloc,
	//    FAudioFreeFunc customFree,
	...
	//   */
	//   FAudio_SetEngineProcedureEXT::proc(
	//    audio:FAudio,
...
/* Callbacks */
 ...
 typedef void (FAUDIOCALL *FAudioEngineCallEXT)(audio:FAudio, float *output);
 typedef void (FAUDIOCALL *FAudioEngineProcedureEXT)(FAudioEngineCallEXT defaultEngineProc, audio:FAudio, float *output, void *user);
 ...
 typedef struct FAudioIOStream
 {
  ...
  FAudio_close(FAudioIOStream *io);
